...
export default function Cases({ cases }: PropsType) {
  ...
  const renderInfoBlock = (items: InfoBlockType[]) => {
    if (!items || items.length === 0) {
      return <SwiperSlide>Нет информации для отображения</SwiperSlide>;
    }
    ...
    return items?.map((item, index) => (
      <SwiperSlide className={style.wrapped} key={index}>
        <div
          className={style.img}
          style={{ backgroundImage: `url(${item.img || hidden})` }}
        ></div>
  ...
  const renderCases = cases.map((item, index) => {
    ...
    const otherTitleWords = titleWords
  ...
  const prevSlide = () => {
    if (activeItemIndex !== 0) setActiveItemIndex((prev) => prev - 1);
  };

  const nextSlide = () => {
    if (activeItemIndex < cases.length - 1) {
          ...
          <div className={style["right-column"]}>
            {!mobileView && (
              <div className={style["tools-menu"]}>
                <button ref={prevBtn} onClick={prevSlide}>
                  <ArrowLeft />
                </button>
                <button ref={nextBtn} onClick={nextSlide}>
                  <ArrowRight />
                </button>
              </div>
            )}
